...
<DatePublished path="system-design/autocomplete" />
...
Below, I've built an interactive autocomplete component using React and Tailwind CSS. The suggestions are retrieved from a Supabase `places` table when the input reaches 3 characters or more.
...
- "t"
- "to"
- "tor" (API call triggers here)
- "toro"
- "toron"
- "toront"
- "toronto"
- Remove one character at a time back to "tor"
- Add one character at a time forward to "toronto"
```
...
3. **Action** - `fetchPlaces` function responsible for retrieving data from Supabase. The response is shaped to match the Google Places API structure.
